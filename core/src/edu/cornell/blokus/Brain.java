            ...
            if(gp != null) {
                ...
                for (int i = 0; i < p.rotations; i++) {
                    for (int j = 0; j < p.corners[i].length; j++) {
                        ...
                        int x = ox - pair.x;
                        int y = oy - pair.y;
                        if (validPartial(p, i, x, y, grid, pa.tile)) {
                            possibleMoves.add(new GamePiece(x, y, i, p, pa.tile));
                        }
                    }
                }
            }
...
*/
    ...
    public static Array<GamePiece> getAllMoves(PlayerArea pa, GameMode.Tile[][] grid){
        Array<Pair> corners = getAllCorners(grid, pa.tile);
                ...
                if(gp != null){
                    ...
                    for(int i=0; i<p.rotations; i++){
                        for(int j=0; j<p.solids[i].length; j++){
                            ...
                            int x = co.x - pair.x;
                            int y = co.y - pair.y;
                            if(validAll(p, i, x, y, grid, pa.tile)){
                                possibleMoves.add(new GamePiece(x, y, i, p, pa.tile));
                            }
                        }
                    }
